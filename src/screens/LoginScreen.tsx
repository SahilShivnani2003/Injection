...
// ── Individual OTP Cell ──────────────────────────────────────────────────────
const OtpCell: React.FC<{ char: string; active: boolean; filled: boolean }> = ({ char, active, filled }) => {
  const borderAnim = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    Animated.timing(borderAnim, { toValue: active ? 1 : 0, duration: 180, useNativeDriver: false }).start();
  }, [active]);
  const borderColor = borderAnim.interpolate({ inputRange: [0, 1], outputRange: ['#D8E8EE', Colors.gradientStart] });
  return (
...
const LoginScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const mobileBorderColor = mobileBorder.interpolate({ inputRange: [0, 1], outputRange: ['#D8E8EE', Colors.gradientStart] });
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.timing(sheetY, { toValue: 0, duration: 640, delay: 160, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      Animated.timing(sheetOp,{ toValue: 1, duration: 640, delay: 160, useNativeDriver: true }),
    ]).start();
  }, []);

  useEffect(() => {
    Animated.timing(mobileBorder, { toValue: mobileActive ? 1 : 0, duration: 180, useNativeDriver: false }).start();
  }, [mobileActive]);

  useEffect(() => {
    if (otpSent) {
      Animated.parallel([
        Animated.timing(otpSlide, { toValue: 1, duration: 400, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
  ...
  const handleSendOtp = () => {
    if (mobile.length !== 10) { Alert.alert('Invalid Number', 'Please enter a valid 10-digit mobile number.'); return; }
    setOtpSent(true);
    Alert.alert('OTP Sent', `OTP sent to +91 ${mobile}`);
  };

  const handleLogin = () => {
    if (!agreed) { Alert.alert('Terms Required', 'Please accept the terms and conditions.'); return; }
    if (otp.length < 4) { Alert.alert('Invalid OTP', 'Please enter the correct OTP.'); return; }
  ...
  return (
    <View style={styles.root}>
      ...
      >
        ...
        <Animated.View style={{ opacity: logoOp, transform: [{ translateY: logoY }], alignItems: 'center' }}>
      ...
      >
        <Animated.View style={[styles.sheet, { opacity: sheetOp, transform: [{ translateY: sheetY }] }]}>
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" bounces={false}>
            ...
            <Text style={styles.sheetSub}>Enter your mobile number to receive an OTP</Text>

            {/* Mobile field */}
            <Text style={styles.fieldLabel}>Mobile Number</Text>
            <Animated.View style={[styles.mobileRow, { borderColor: mobileBorderColor }]}>
              ...
              />
              {mobile.length === 10 && (
                <View style={styles.validBadge}><Text style={styles.validTick}>✓</Text></View>
              )}
            </Animated.View>
            ...
            {!otpSent ? (
              ...
              <View style={styles.sentRow}>
                ...
                <TouchableOpacity onPress={() => { setOtp(''); setOtpSent(false); }}>
            ...
            {/* OTP entry */}
            <Animated.View style={{ opacity: otpOp, transform: [{ translateY: otpTranslateY }] }}>
              <Text style={[styles.fieldLabel, { marginTop: 24 }]}>One-Time Password</Text>
              <TextInput
                ref={otpRef}
              ...
              />
              <TouchableOpacity style={styles.otpRow} onPress={() => otpRef.current?.focus()} activeOpacity={1}>
                {otpCells.map((c, i) => (
                  <OtpCell key={i} char={c.char} active={c.active} filled={c.filled} />
                ))}
              </TouchableOpacity>
            </Animated.View>
            ...
            >
              <LinearGradient
                colors={canLogin ? [Colors.accent, Colors.accentDark] : ['#E8F0F4', '#DDE8EE']}
                start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                style={styles.btnGrad}
              >
                <Text style={[styles.loginBtnText, !canLogin && styles.loginBtnTextOff]}>
                  Confirm & Login
                </Text>
              </LinearGradient>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  crossH: { position: 'absolute', width: 32, height: 9, borderRadius: 4.5, backgroundColor: Colors.white },
  crossV: { position: 'absolute', width: 9, height: 32, borderRadius: 4.5, backgroundColor: Colors.white },
  brandName: {
  ...
  brandTagline: {
    ...
    letterSpacing: 0.4, marginTop: 6, fontWeight: '400',
  },
  ...
  sentBadge: {
    backgroundColor: '#EAF9F4', paddingHorizontal: 12,
  ...
  resendText: {
    fontSize: 13, color: Colors.gradientStart,
  ...
  otpCell: {
    ...
    borderWidth: 1.5, backgroundColor: '#F8FBFC',
    alignItems: 'center', justifyContent: 'center',
  },
  ...
  footerNote: {
    ...
    color: '#A8BEC8', marginTop: 20,
    lineHeight: 17, paddingHorizontal: 10,
  },
});
